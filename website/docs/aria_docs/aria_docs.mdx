---
sidebar_position: 1
title: Project Aria Docs Case Study
description: Working with a much larger team of SMEs we created Project Aria Docs (originally called Project Aria Tools) within a very tight timeline.
---

# Project Aria Docs Case Study

## Problem
Aria Data Tools was created in a very small window of time for a specific release. After the success of Aria Data Tools, we needed to create a much larger website that catered to multiple audiences. Research Partner only documentation was going to be made open source. The site needed to be able to support a lot more tooling (C++, Python, apps and a SDK), use cases and data releases.

## Actions
Working with a much larger team of SMEs we created Project Aria Docs (originally called Project Aria Tools) within a very tight timeline. This site was then added to and evolved over time. I created templates to drive clarity and consistency in associated content (such as the [HOT3D README](https://github.com/facebookresearch/hot3d/commit/a23c9dc4aa1e0679a5089af376e7cbd32434c2f5)).

## Outcomes
Enormously positive feedback from our users about the quality of our documentation, especially when compared to other documentation in the field. It is really nice to hear PhD students loudly exclaim "Your documentation is amazing, it saved me!" etc.

Engineers and researchers have thanked me for raising the standards around documentation and that they'll take these principles forward into future projects.

## Sample Pages

* [Introduction to Project Aria Docs](/aria_docs/intro.md)
* [Project Aria FAQ](/aria_docs/faq.mdx)
* [Aria Research Kit Release Notes](/aria_docs/sw_release_notes.mdx)
* [Recording Profiles](/aria_docs/recording_profiles.mdx)
* [Timestamp Definitions](/aria_docs/timestamps_in_aria_vrs.mdx)
* [Getting Started With Project Aria Glasses](/aria_docs/ARK_quickstart.mdx)
* [Time-Synchronized Recordings with Multiple Aria Glasses](/aria_docs/ticsync.mdx)
* [HOT3D README Commit](https://github.com/facebookresearch/hot3d/commit/a23c9dc4aa1e0679a5089af376e7cbd32434c2f5)
    * Commit showing how content has been improved and made consistent with the larger Project Aria product family
* Machine Perception Services (MPS) - a cloud-based service
    * [MPS CLI - Getting Started](/aria_docs/mps/mps_cli_getting_started.mdx)
    * [MPS CLI Guide](/aria_docs/mps/mps_cli_guide.mdx)
    * [Eye Gaze MPS](/aria_docs/mps/mps_eye_gaze.mdx)
    * [Eye Gaze Calibration](/aria_docs/mps/eye_gaze_calibration.mdx)
    * [MPS Data Processing](/aria_docs/mps/mps_processing.mdx)
    * [MPS Troubleshooting](/aria_docs/mps/mps_troubleshooting.mdx)

[Project Aria Docs site](https://facebookresearch.github.io/projectaria_tools/docs/intro)